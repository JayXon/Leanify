...
size_t Mime::Leanify(size_t size_leanified /*= 0*/)
{
    ...
    while (p_read < fp_ + size_)
    {
        ...
        uint8_t* encoding_magic = std::search(p_read, fp_ + size_, magic.begin(), magic.end(), [](char a, char b)
                                              {
        ...
        uint8_t* start = std::search(encoding_magic + magic.size(), fp_ + size_, double_crlf.begin(), double_crlf.end()) + double_crlf.size();
        ...
        if (is_verbose)
        {
            cout << string(reinterpret_cast<char *>(start), 8) << "... found at offset 0x" << std::hex << start - fp_ << std::dec << endl;
        }
